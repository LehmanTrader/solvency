...
/** Monthly totals are whole dollars: $24, $423, $2.4k. */
export const moneyMonth = (n: number) => n >= 1000 ? `$${(n / 1000).toFixed(1)}k` : `$${Math.round(n)}`;

const esc = (s: string) => String(s).replace(/[&<>"']/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]!));
const r1 = (n: number) => Math.round(n * 10) / 10;
const pct = (p: number) => `${Math.round(p * 100)}%`;
const CHAR = 7.25; // JetBrains Mono advance at 12px
const trunc = (s: string, px: number, size = 12) => {
  const n = Math.floor(px / (CHAR * size / 12));
  return s.length > n ? s.slice(0, Math.max(1, n - 1)) + '…' : s;
};

/** One <pattern> per basis; referenced by every hatched bar. */
const hatch = (basis: Basis) =>
  `<pattern id="hatch-${basis}" patternUnits="userSpaceOnUse" width="6" height="6" patternTransform="rotate(45)" style="color:var(--color-${basis})">` +
  `<line x1="0" y1="0" x2="0" y2="6" stroke="currentColor" stroke-width="2"${basis === 'stale' ? ' stroke-dasharray="2 2"' : ''}/></pattern>`;
...
export interface RankedOpts {
  ...
  highlight?: string;
  /** index at which rows are hidden behind the caller's disclosure; null = all */
  title?: string;
}
...
export function rankedBars(rowsIn: ChartRow[], o: RankedOpts): string {
  ...
  // columns
  const cmpW = compact ? 0 : 58, monthW = compact ? 0 : 64, solvedW = compact ? 0 : 68, gap = 14;
  ...
  // non-lead measured bars are dimmed by a CSS class (theme-specific opacity keeps them ≥ 3:1)
  const fill = (i: number) => basis === 'measured'
    ? `fill="var(--color-measured)"${i === 0 ? '' : ' data-dim="1"'}`
    : `fill="url(#hatch-${basis})" stroke="var(--color-${basis})" stroke-width="1"`;

  const header = compact ? '' :
    `<g class="t3" font-size="9.5" aria-hidden="true">` +
  ...
  const body = rows.map((r, i) => {
    ...
    const cmp = r.compare ? (compact
      ? `<a class="cmp" href="${esc(r.compare)}" aria-label="Compare ${esc(r.name)} head to head"><text x="${w - 118}" y="${rowH - 14}" font-size="10" class="t3">vs ›</text></a>`
      : `<a class="cmp" href="${esc(r.compare)}" aria-label="Compare ${esc(r.name)} head to head"><text x="${w}" y="${rowH / 2 + 4}" text-anchor="end" font-size="10" class="t3">compare ›</text></a>`) : '';
    const rail = i === 0 ? `<rect x="0" y="4" width="2" height="${rowH - 8}" fill="var(--color-${basis})"/>` : '';
    ...
    return compact
      ? `<g class="${cls}" data-id="${esc(r.id)}" role="listitem" aria-label="${esc(label)}" style="transform:translateY(${y}px)">` +
        ...
        `<a href="${esc(r.href)}"><text class="name" x="8" y="15" font-size="${fs}">${esc(name)}</text></a>` +
        `<text class="v c-${basis}" data-f="solved" x="${xSolved}" y="15" text-anchor="end" font-size="${fs}" font-weight="700">${money(r.cost)}</text>` +
        ...
        cmp +
        `<text class="t2" data-f="month" x="${xMonth}" y="${rowH - 13}" text-anchor="end" font-size="11">${month}/mo</text></g>`
      : `<g class="${cls}" data-id="${esc(r.id)}" role="listitem" aria-label="${esc(label)}" style="transform:translateY(${y}px)">` +
        `<title>${esc(detail)}</title>${rail}` +
        `<a href="${esc(r.href)}"><text class="name" x="10" y="${rowH / 2 + 4}" font-size="${fs}">${esc(name)}</text></a>` +
        ...
        `<text class="v c-${basis}" data-f="solved" x="${xSolved}" y="${rowH / 2 + 4}" text-anchor="end" font-size="${fs}" font-weight="700">${money(r.cost)}</text>` +
        `<text class="v" data-f="month" x="${xMonth}" y="${rowH / 2 + 4}" text-anchor="end" font-size="${fs}">${month}</text>` +
        cmp + `</g>`;
  }).join('');
  ...
  return `<svg class="chart ranked${compact ? ' compact' : ''}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}" role="list" aria-label="${esc(title)}" data-sig="${esc(sig)}" data-basis="${basis}">` +
    `<title>${esc(title)}</title><desc>${esc(desc)}</desc><defs>${hatch(basis)}</defs>${header}${body}</svg>`;
}
...
export function patchRanked(container: Element, html: string): void {
  ...
  next.querySelectorAll<SVGGElement>('g.row').forEach((nr) => {
    ...
    const nrail = nr.firstElementChild?.tagName === 'rect' && !nr.firstElementChild.classList.contains('track') ? nr.firstElementChild : nr.querySelector(':scope > title + rect');
...
export function scatterPareto(rows: ChartRow[], o: ScatterOpts): string {
  ...
  const ml = 54, mr = compact ? 14 : 24, mt = 14, mb = 34;
  ...
  for (let p = xmin; p <= xmax + 1e-9; p = r1(p + 0.1)) {
    gx.push(`<line class="grid" x1="${r1(X(p))}" y1="${mt}" x2="${r1(X(p))}" y2="${mt + ph}"/>`);
    xt.push(`<text class="t3" x="${r1(X(p))}" y="${h - mb + 16}" text-anchor="middle" font-size="10">${Math.round(p * 100)}%</text>`);
  }
  const yt: string[] = [];
  for (let c = ymin; c <= ymax * 1.0001; c *= 10) {
    yt.push(`<line class="grid" x1="${ml}" y1="${r1(Y(c))}" x2="${ml + pw}" y2="${r1(Y(c))}"/>` +
      `<text class="t3" x="${ml - 8}" y="${r1(Y(c)) + 3.5}" text-anchor="end" font-size="10">$${c < 1 ? c.toFixed(1) : c}</text>`);
  }
  ...
  // then the rest; each tries above-right, below-right, above-left, below-left.
  const FS = 10.5, LH = 12;
  const order = pts.slice().sort((a, b) => (Number(onFront.has(b.id) || b.id === o.lead) - Number(onFront.has(a.id) || a.id === o.lead)) || a.cost - b.cost);
  // every mark is an obstacle, so a label never sits on a point
  const placed: { x0: number; y0: number; x1: number; y1: number }[] = pts.map((p) => ({ x0: X(p.pass) - 7, y0: Y(p.cost) - 7, x1: X(p.pass) + 7, y1: Y(p.cost) + 7 }));
  ...
  for (const p of order) {
    ...
    const x = r1(X(p.pass)), y = r1(Y(p.cost));
    const lw = p.name.length * FS * 0.61;
    const tries = [
      { x: x + 9, y: y - 7, anchor: 'start' }, { x: x + 9, y: y + 16, anchor: 'start' },
      { x: x - 9, y: y - 7, anchor: 'end' }, { x: x - 9, y: y + 16, anchor: 'end' },
      { x: x + 9, y: y - 19, anchor: 'start' }, { x: x + 9, y: y + 28, anchor: 'start' },
      { x: x - 9, y: y - 19, anchor: 'end' }, { x: x - 9, y: y + 28, anchor: 'end' },
      { x, y: y - 11, anchor: 'middle' }, { x, y: y + 20, anchor: 'middle' },
    ];
    let pick: typeof tries[number] | null = null;
    for (const t of tries) {
      const x0 = t.anchor === 'start' ? t.x : t.anchor === 'end' ? t.x - lw : t.x - lw / 2, x1 = x0 + lw, y0 = t.y - LH, y1 = t.y + 2;
      if (x0 < ml - 4 || x1 > w - 2 || y0 < mt) continue;
      if (placed.some((b) => x0 < b.x1 && x1 > b.x0 && y0 < b.y1 && y1 > b.y0)) continue;
      pick = t; placed.push({ x0, y0, x1, y1 }); break;
    }
    // a frontier/lead label is never dropped: fall back to above-right
    if (!pick && (onFront.has(p.id) || p.id === o.lead)) pick = tries[0];
    label.set(p.id, pick);
  }
  const marks = pts.slice().sort((a, b) => (a.basis === 'measured' ? 1 : 0) - (b.basis === 'measured' ? 1 : 0)).map((p) => {
    ...
    const mark = p.basis === 'measured'
      ? `<circle class="pt-measured" cx="${x}" cy="${y}" r="${onFront.has(p.id) ? 5 : 4}"/>`
      : p.basis === 'modelled'
        ? `<circle class="pt-modelled" cx="${x}" cy="${y}" r="4.5"/>`
        : `<circle class="pt-stale" cx="${x}" cy="${y}" r="4.5"/>`;
    ...
    return `<a href="${esc(p.href)}" aria-label="${esc(tip)}"><title>${esc(tip)}</title>${mark}${text}</a>`;
  }).join('');

  const legend = compact ? '' :
    `<g font-size="10" class="t3"><circle class="pt-measured" cx="${ml + 8}" cy="${mt + 8}" r="4"/><text x="${ml + 18}" y="${mt + 11.5}">MEASURED</text>` +
    `<circle class="pt-modelled" cx="${ml + 96}" cy="${mt + 8}" r="4"/><text x="${ml + 106}" y="${mt + 11.5}">MODELLED — not on the frontier by construction</text></g>`;
  ...
  return `<svg class="chart scatter" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}" role="img" aria-label="${esc(title)}">` +
    ...
    xt.join('') +
    `<text class="t3" x="${ml + pw}" y="${h - 4}" text-anchor="end" font-size="9.5">PASS RATE →</text>` +
    `<text class="t3" transform="translate(12 ${mt + ph - 26}) rotate(-90)" font-size="9.5">$ / SOLVED TASK (log) →</text>` +
    (path ? `<path class="frontier" d="${path}"/>` : '') + marks + legend + `</svg>`;
}
...
export function volumeLines(seriesIn: Series[], o: LinesOpts): string {
  ...
  const ml = 58, mr = compact ? 14 : 150, mt = 14, mb = 34;
  const pw = w - ml - mr, ph = h - mt - mb;
  ...
  const grid: string[] = [];
  for (let v = VOL_MIN; v <= VOL_MAX; v *= 10)
    grid.push(`<line class="grid" x1="${r1(X(v))}" y1="${mt}" x2="${r1(X(v))}" y2="${mt + ph}"/>` +
      `<text class="t3" x="${r1(X(v))}" y="${h - mb + 16}" text-anchor="middle" font-size="10">${kfmt(v)}</text>`);
  for (let c = ymin; c <= ymax * 1.0001; c *= 10)
    grid.push(`<line class="grid" x1="${ml}" y1="${r1(Y(c))}" x2="${ml + pw}" y2="${r1(Y(c))}"/>` +
      `<text class="t3" x="${ml - 8}" y="${r1(Y(c)) + 3.5}" text-anchor="end" font-size="10">${mfmt(c)}</text>`);
  ...
  const lines = series.map((s, i) => {
    ...
    const label = compact ? '' : `<text class="${k}" x="${ml + pw + 8}" y="${y1 + 4}" font-size="10.5">${esc(trunc(s.name, mr - 12, 10.5))}</text>` +
      `<text class="t3" x="${ml + pw + 8}" y="${y1 + 16}" font-size="9">${BASIS_WORD[s.basis]}</text>`;
    return `<g aria-label="${esc(`${s.name}, ${s.basis}: ${money(at)} a month at ${vol.toLocaleString()} tasks`)}"><title>${esc(`${s.name} · ${BASIS_WORD[s.basis]} · ${money(s.perTask)} per solved task`)}</title>` +
      ...
      `<text class="${k}" data-f="at-${i}" x="${vx + (vx > ml + pw * 0.7 ? -8 : 8)}" y="${r1(Y(at)) - 7}" text-anchor="${vx > ml + pw * 0.7 ? 'end' : 'start'}" font-size="11" font-weight="700">${money(at)}</text>${label}</g>`;
  }).join('');

  const legend = !compact ? '' : `<g font-size="10">${series.map((s, i) => `<text class="s${i + 1}" x="${ml + 4 + i * 0}" y="${mt + 12 + i * 13}">— ${esc(trunc(s.name, pw - 60, 10))} · ${BASIS_WORD[s.basis]}</text>`).join('')}</g>`;
  ...
  return `<svg class="chart lines" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}" role="img" aria-label="${esc(title)}" data-px0="${ml}" data-px1="${ml + pw}" style="touch-action:none">` +
    ...
    `<line class="axis" x1="${ml}" y1="${mt + ph}" x2="${ml + pw}" y2="${mt + ph}"/><line class="axis" x1="${ml}" y1="${mt}" x2="${ml}" y2="${mt + ph}"/>` +
    `<text class="t3" x="${ml + pw}" y="${h - 4}" text-anchor="end" font-size="9.5">TASKS / MONTH (log) →</text>` +
    `<text class="t3" transform="translate(12 ${mt + ph - 26}) rotate(-90)" font-size="9.5">$ / MONTH (log) →</text>` +
    `<line class="marker" x1="${vx}" y1="${mt}" x2="${vx}" y2="${mt + ph}"/>` +
    `<text class="t2" x="${vx}" y="${mt + ph + 28}" text-anchor="middle" font-size="10">${vol.toLocaleString()} tasks</text>` +
    lines + legend + `</svg>`;
}
