...
export function quoteBuildPlan(
  ...
  for (const role of plan.roles) {
    ...
    if (!finiteNonnegative(model.input_per_mtok) || !finiteNonnegative(model.output_per_mtok)) {
    ...
    const cacheReadRate = override?.cacheReadPerMtok ?? model.cached_input_per_mtok ?? inputRate;
    const cacheWriteRate = override?.cacheWritePerMtok ?? 0;
  ...
  if (errors.length) {
    return {
      ...
      errors, warnings, missing, buildAttemptCostUsd: null,
  ...
  return {
    ...
    errors, warnings, missing, buildAttemptCostUsd, variableCostPerSuccessfulBuildUsd,
